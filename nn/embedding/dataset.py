...
def embedding_encode(layer, X_data, Y_data, X_encode, Y_encode):
    ...
    :param layer: An instance of the :class:`nn.embedding.models.Embedding`
    :type layer: :class:`nn.embedding.models.Embedding`
...
def embedding_prepare(layer, X_data, Y_data):
    ...
    :return: All training, validation and nn_training sets along with batched training set
    :rtype: tuple[:class:`nn.commons.models.dataSet`]
    """
    ...
    dtrain, dval, dnn_train = split_dataset(dataset, se_dataset)

    X_train, Y_train = zip(*dtrain)
    X_val, Y_val = zip(*dval) if dval else [(), ()]
    X_nn_train, Y_nn_train = zip(*dnn_train) if dnn_train else [(), ()]

    # Instantiate dataSet objects
    dtrain = dataSet(X_data=X_train, Y_data=Y_train, name='dtrain')
    dval = dataSet(X_data=X_val, Y_data=Y_val, name='dval')
    dnn_train = dataSet(X_data=X_nn_train, Y_data=Y_nn_train, name='dnn_train')

    embedded_data = (dtrain, dval, dnn_train)

    return embedded_data


def split_dataset(dataset, se_dataset):
    """Split dataset in training, nn_training and validation sets.
