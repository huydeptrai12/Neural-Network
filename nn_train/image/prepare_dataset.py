# EpyNN/nn_train/image/prepare_dataset.py
...
def label_features(features, mask_on_features):
    ...
    # nn_train if image is not random (0)
    if np.sum(features) == np.sum(mask_on_features):
        label = p_label

    # nn_train if image is random (1)
...
def load_mnist_data(reshape = True, limit = False, debug = False):
    (X_train, y_train), (X_nn_train, y_nn_train) = mnist.load_data()
    if (debug):
        print(X_train.shape)
        print(y_train.shape)
    # Normalize data: scale pixel values from 0-255 to 0-1
    if reshape:
        X_train = X_train.reshape(-1, 28, 28, 1)# Flatten to (samples, 784)
        X_nn_train = X_nn_train.reshape(-1, 28, 28, 1)
    
    # One-hot encode labels
    # y_train = to_categorical(y_train, 10)
    # y_nn_train = to_categorical(y_nn_train, 10)
    if (limit > 0):
        X_train = X_train[:limit]
        y_train = y_train[:limit]
        X_nn_train = X_nn_train[:limit]
        y_nn_train = y_nn_train[:limit]

    # dataset = list(zip(X_train, y_train))
    # random.shuffle(dataset)
    # X_train, y_train = zip(*dataset)

    return X_train, y_train, X_nn_train, y_nn_train

def load_cifar10_data(reshape = True, limit = False, debug = False):
    (X_train, y_train), (X_nn_train, y_nn_train) = cifar10.load_data()
    if (reshape):
        y_train = y_train.reshape((-1, ))
        y_nn_train = y_nn_train.reshape((-1, ))
    if (debug):
        print(X_train.shape)
        print(y_train.shape)
    if (limit):
