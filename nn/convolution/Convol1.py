...
def optimized_convolution_forward(layer, A):
    ...
    Z = np.zeros((layer.d['m'], layer.d['oh'], layer.d['ow'], layer.d['u']))
...
pool1 = Convolution(unit_filters=32, filter_size=(3, 3), strides=(1, 1))
pool1.np_rng = generator
pool1.init(input_data)
...
@profile
def run_memory_test():
    forward1 = optimized_convolution_forward(pool1, input_data)
    output_data = np.random.rand(*forward1.shape)
    backward1 = optimized_convolution_backward(pool1, output_data)
