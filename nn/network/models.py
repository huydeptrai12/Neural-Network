...
from nn.commons.io import (
    encode_dataset,
    scale_features,
)
from nn.commons.library import write_model
from nn.commons.loss import loss_functions
from nn.commons.models import dataSet
from nn.network.report import (
    model_report,
    initialize_model_report,
    single_batch_report,
)
from nn.commons.plot import pyplot_metrics
from nn.network.initialize import (
    model_assign_seeds,
    model_initialize,
    model_initialize_exceptions,
)
from nn.network.hyperparameters import (
    model_hyperparameters,
    model_learning_rate,
)
from nn.network.evaluate import model_evaluate
from nn.network.forward import model_forward
from nn.network.backward import model_backward
from nn.network.training import model_training
from nn.settings import se_hPars


class nn:
    """
    Definition of a Neural Network prototype following the nn scheme.

    :param layers: Network architecture.
    :type layers: list[Object]

    :param name: Name of network, defaults to 'nn'.
    :type name: str, optional
    """

    def __init__(self, layers, name='nn'):
        ...
        :vartype embedding: :class:`nn.embedding.models.Embedding`
    ...
    def forward(self, X):
        """Wrapper for :func:`nn.network.forward.model_forward()`.
    ...
    def backward(self, dA):
        """Wrapper for :func:`nn.network.backward.model_backward()`.
    ...
    def initialize(self, loss='MSE', se_hPars=se_hPars, metrics=['accuracy'], seed=None, params=True, end='\n'):
        """Wrapper for :func:`nn.network.initialize.model_initialize()`. Perform a dry epoch including all but not the parameters update step.

        :param loss: Loss function to use for training, defaults to 'MSE'. See :py:mod:`nn.commons.loss` for built-in functions.
        :type loss: str, optional

        :param se_hPars: Global hyperparameters, defaults to :class:`nn.settings.se_hPars`. If local hyperparameters were assigned to one layer, these remain unchanged.
        :type se_hPars: dict[str: float or str], optional

        :param metrics: Metrics to monitor and print on terminal report or plot, defaults to ['accuracy']. See :py:mod:`nn.commons.metrics` for built-in metrics. Note that it also accept loss functions string identifiers.
    ...
    def train(self, epochs, verbose=None, init_logs=True, report = True):
        """Wrapper for :func:`nn.network.training.model_training()`. Apart, it computes learning rate along learning epochs.
        ...
        model_training(self, report)

        return None

    def evaluate(self):
        """Wrapper for :func:`nn.network.evaluate.model_evaluate()`. Good spot for further implementation of early stopping procedures.
        """
        model_evaluate(self)
    ...
    def write(self, path=None):
        ...
        :param path: Path to write the model on disk, defaults to `None` which writes in the `models` subdirectory created from :func:`nn.commons.library.configure_directory()`.
    ...
    def batch_report(self, batch, A):
        """Wrapper for :func:`nn.network.report.single_batch_report()`.
        """
    ...
    def report(self):
        """Wrapper for :func:`nn.network.report.model_report()`.
        """
        model_report(self)
    ...
    def plot(self, pyplot=True, path=None):
        """Wrapper for :func:`nn.commons.plot.pyplot_metrics()`. Plot metrics from model training.

        :param pyplot: Plot of results on GUI using matplotlib.
        :type pyplot: bool, optional

        :param path: Write matplotlib plot, defaults to `None` which writes in the `plots` subdirectory created from :func:`nn.commons.library.configure_directory()`. To not write the plot at all, set to `False`.
    ...
    def predict(self, X_data, X_encode=False, X_scale=False):
        ...
        :rtype: :class:`nn.commons.models.dataSet`
        """
