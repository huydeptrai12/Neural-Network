...
def headers_logs(model, colors):
    ...
    :param model: An instance of nn network object.
    :type model: :class:`nn.network.models.nn`
    ...
    len_dsets = len(model.embedding.dsets)    # Number of active datasets (dtrain, dnn_train, dval or dtrain, dnn_train or dtrain)
    ...
    for i, s in enumerate(list(metrics.keys())):
        ...
        # There may be one non-empty validation and nn_training set
        if len_dsets == 3:
            headers.append('\n' + colored('dnn_train', colors[i], attrs=['bold']))
...
def current_logs(model, colors):
    ...
    """
    metrics = model.metrics          # Contains metrics + cost computed along training epochs
    dsets = model.embedding.dsets    # Active datasets (dtrain, dval, dnn_train or dtrain, dval or dtrain)
...
def network_logs(network):
    ...
    :param network: Attribute of an instance of nn network object.
...
def layers_lrate_logs(layers):
    ...
    :param layers: Attribute of an instance of nn network object.
...
def dsets_samples_logs(dsets, se_dataset):
    ...
    :type dsets: list[:class:`nn.commons.models.dataSet`]
